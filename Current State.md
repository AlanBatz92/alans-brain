...
| UAP Shape Census | `ufo-shapes.html` | Growing census of UFO/UAP craft **shape** descriptions across ingested sources (disc/sphere/triangle/cigar/Tic-Tac/…), every mention cited back to source + a **per-source drill-down**. Vanilla page (`us-` prefix, self-contained styles) reads `data/ufo-shapes/summary.json`; the local `ufo-shapes/` Python toolkit produces the JSON. **Not yet nav-linked or in the Tech Stack** (direct URL only). Current corpus: **6 books ingested** (Jung 1958 · Vallee 1969 · Hastings 2008 · Kean 2010 · Coulthart 2021 · Graff 2023) → **1,083 high-confidence shape mentions across 11 shapes**. Tier 2: Kean + Graff; Tier 3: the rest. Next: snippet drill-down ("the receipts"), then link from `paranormal.html` + nav + Tech Stack node. Full design + phased next-steps: `PLAN-ufo-shapes.md`. |
