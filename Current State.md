...
| UAP Shape Census | `ufo-shapes.html` | A **vernacular / zeitgeist** census of how the culture *describes* UFO/UAP craft by **shape** (disc/sphere/triangle/cigar/Tic-Tac/…) across intentionally-curated sources — a read of *how we talk and think* about the phenomenon, **not** a physical-truth claim — every mention cited back to source. **(Reframed 2026-06-29; Alan is rebuilding the corpus from scratch locally with more sources + Jonathan Weygandt interviews.)** **Snippet drill-down (live 2026-06-27):** tap a shape (or a source's shape chip) → modal of the actual cited passages (matched term highlighted, source · year · tier · locator), lazy-loading `mentions.json`, with a source-filter pill row. **De-dupes repeated passages (2026-06-29):** same-passage mentions (repeat 280-char windows of one paragraph, or a sentence a segmenter split across two locators) collapse into one card with a **×N** badge — merged by a reliable locator (`LOC_COUNT` density ≤ 6, so junk catch-all locators like Dennett's 85-mention `ch:1#para:0` don't over-merge distinct sightings) **or** snippet text overlap (`wordOverlap` ≥ 0.7); the modal subtitle/pills count distinct passages while the leaderboard still counts every mention (methodology panel says so). Front-end only (`dedupePassages`/`highlightTerms` in `ufo-shapes.html`), no data change. **Vernacular "described as" view (2026-06-29):** each canonical shape rolls up the colloquial words that resolved to it (from `raw_term` → `summary.json`'s new `terms_by_shape`, emitted by `build.py`) — a glanceable caption under every leaderboard bar ("Triangle — described as: triangular · wedge-shaped · arrowhead · …") **plus** an interactive teal term-filter row in the drill-down (tap a colloquialism → just those passages; distinct from the green source pills). The local **`ufo-shapes/candidates.py`** scans segments for "<word>-shaped / in the shape of a ___ / looked like / resembled" and reports descriptors **not yet in `shapes.json`** ("ice-cream cone", "propane tank") so the vocabulary is easy to grow. Vanilla page (`us-` prefix, self-contained styles) reads `data/ufo-shapes/summary.json`; the local `ufo-shapes/` Python toolkit produces the JSON. **Reached through the Paranormal hub** (a featured card on `paranormal.html` with live counts pulled from `summary.json` → links here; a "← Back to Paranormal" link returns), matching how sub-pages nest — no separate top-nav entry. Not in the Tech Stack graph yet (follow-up). Current corpus (2026-06-27): **11 sources ingested** (Jung 1958 · Vallee 1969 · Hastings 2008 · Mack 2009 · Kean 2010 · Dolan "21st Century Mind" 2014 · Dennett "Onboard" 2020 · Coulthart 2021 · Kelleher "Skinwalkers" 2021 · Graff 2023 · Dolan "A History of USOs" 2025) → **1,852 AI-confirmed shape mentions across 11 shapes** (the local `classify.py` pass dropped ~32% of lexical hits as non-craft noise). Tier 2: Kean, Graff, Mack, Kelleher; Tier 3: the rest. Next: a Tech Stack node. Full design + phased next-steps: `PLAN-ufo-shapes.md`. |
