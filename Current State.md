...
| My Week | `weather.html` / `weather.js` | Weather outlook + running/drone/tan scoring. **Redesigned 2026-06-17** to a **hero "Today" card + "best this week" chips + roomy 6-day list** (was a cramped 7-column strip); tap any day/chip → detail drawer. The drawer (also 2026-06-17) shows a **single shared `Conditions` block** then per-activity rating · best window · hourly bars · tap-to-open score breakdown (no repeated metrics). Rating colors are a 4-step ramp **green/blue/amber/red** (Good = blue `--blue`, not teal). **Running score** uses apparent temp + **dew point** (not raw humidity), UV dropped — `computeRunScore()`/`getDewPoint()`. Assets `?v=w3`. Passphrase-gated until it's public. |
